...
default codebase is invented. The SIQshift project stays on the roster row as a label
...
- A working directory can contain a user name, so it's shown only to the owning user and org
  ...
  last segment, a name like `siqshift` - which says which codebase an agent worked in without
