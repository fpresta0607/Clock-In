...
- **Per-app breakdown.** The same stats view lists active time per foreground process (top 10, then a count of the rest) from the `apps` array on `GET /me/stats`, which sums the caller's active segments grouped by process name. Only the executable name is shown — window titles and URLs are never collected — so the user can answer "where did my time go?" without the monitor seeing content.
